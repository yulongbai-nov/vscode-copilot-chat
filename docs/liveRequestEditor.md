...
3. **Endpoint (Raw messages → `IEndpointBody` JSON)**
   ...
   - `IEndpointBody` (what is actually sent) includes:
     ...
     - For models that reject sampling controls (e.g., `o1`/`o1-mini`, `gpt-5.1-codex`), the request builder strips `temperature`, `top_p`, and `n` to avoid `invalid_request_body` errors. The metadata view will omit those keys for such models.
