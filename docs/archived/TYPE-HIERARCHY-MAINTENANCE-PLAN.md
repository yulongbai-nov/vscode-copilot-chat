...
Note: The automation script and workflow described here were retired in favor of the nightly upstream merge job (`.github/workflows/fork-nightly-merge.yml`). This plan remains archived for historical reference.
...
- **M2 – Automation Script**
  - Implemented `script/update-type-hierarchy.sh` to orchestrate sync + rebase + validation (retired).
...
- **M3 – Validation & CI Hooks**
  ...
  - Historically published automation workflow at `.github/workflows/type-hierarchy-maintenance.yml` (retired).
    ```yaml
...
- [ ] `script/update-type-hierarchy.sh` exists and is executable. (Retired; replaced by nightly merge automation.)
...
- [ ] CI workflow exists at `.github/workflows/type-hierarchy-maintenance.yml`. (Retired; replaced by nightly merge automation.)
