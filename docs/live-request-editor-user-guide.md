...
- Models that reject sampling controls (e.g., `o1`, `o1-mini`, `gpt-5.1-codex`) have `temperature` / `top_p` / `n` stripped before send, so the Request Options node omits them.
