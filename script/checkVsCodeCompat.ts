...
try {
	const vscodeVersion = resolveVsCodeVersion(candidates);

	if (!valid(vscodeVersion)) {
		console.error(`[compat] Unable to parse VS Code version "${vscodeVersion}".`);
		process.exit(1);
	}
...
function resolveVsCodeVersion(commands: readonly string[]): string {
